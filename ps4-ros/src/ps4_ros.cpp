...
class PS4_ROS {
 ...
 public:
  ...
  PS4_ROS(ros::NodeHandle &n) {
    ...
    private_nh.param<std::string>("pub_topic", this->pubName,
                                  "/searchbot/p3at/vel_cmd");

    this->chat = n.advertise<geometry_msgs::Twist>(pubName, 1000);
    this->sub =
        n.subscribe<sensor_msgs::Joy>("/joy", 10, &PS4_ROS::subscribePS4, this);
  ...
  bool calibrate() {
    ...
    if ((this->calib1 > this->calib_duration) &&
        (this->calib2 > this->calib_duration)) {
      this->calib = true;
      return true;
    } else
      return false;
 ...
 private:
  ...
  int arrowsX, arrowsY, buttonSq, buttonX, buttonO, buttonTr, buttonTouch, l1,
      r1;
